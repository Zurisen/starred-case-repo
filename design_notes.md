...
- Add extra sanitizing conditions for some of the fields. We assumed user names were just a single field (not separated in Name and Surname), comments possibly containing offensive language. Regions and Countries could also be further sanitized to make sure they belong to an existing class, preventing possible typos or unhandled cases. Timestamp could be splitted into granular date columns (year, quarter, month...) and emails could be anonymized for extra user security.
